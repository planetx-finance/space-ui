...
class Store {
    ...
    this.store = {
      ...
      vaultAssets: [
        ...
        },
        {
          ...
          name: "Roscosmos",
          symbol: "Roscosmos",
          description: "Roscosmos",
          vaultSymbol: "xRoscosmos",
          erc20address: "0xe905987DCE4E13b6D28526DFF2D0a76a837E5E68",
          vaultContractAddress: "0xe905987DCE4E13b6D28526DFF2D0a76a837E5E68",
          ...
          measurement: 1e18,
          price_id: "dify-finance",
        },
        {
          id: "VirginGalactic",
          name: "VirginGalactic",
          symbol: "VirginGalactic",
          description: "Virgin Galactic",
          vaultSymbol: "xVirginGalactic",
          erc20address: "0xdf5e0e81dff6faf3a7e52ba697820c5e32d806a8",
  ...
  buyToken = async (account, amount) => {
    ...
    let transaction = {
      to: "0x281E3789Ec876A24B4e25DcA0b2303C9C9d70187",
      nonce: nonce,
  ...
  _getStatsAPY = (vaultStatistics, asset, callback) => {
    try {
      ...
      const vault = vaultStatistics.filter((stats) => {
        return (
          stats.tokenAddress.toLowerCase() === asset.erc20address.toLowerCase()
        );
      });
  ...
  _getUSDPrices = async () => {
    try {
      const url =
        "https://api.coingecko.com/api/v3/simple/price?ids=usd-coin,dai,true-usd,tether,usd-coin,chainlink,yearn-finance,binance-usd,wrapped-bitcoin,ethereum,nusd,chainlink,aave-link,lp-sbtc-curve,lp-bcurve,curve-fi-ydai-yusdc-yusdt-ytusd&vs_currencies=usd,eth";
  ...
  _calculateDashboard = () => {
    ...
    if (
        ...
        .map((vault) => {
          ...
          const vaultStats = statistics.filter((stats) => {
            return (
              ...
              vault.erc20address.toLowerCase()
            );
          });
          ...
          vault.prices = price;
          vault.usdBalance =
            vault.vaultBalance * vault.pricePerFullShare * vault.prices.usd;
          ...
          vault.vaultGrowth_yearly_usd =
            ((vault.vaultBalance * vault.pricePerFullShare * apy) / 100) *
            vault.prices.usd;

          vault.ethBalance =
            vault.vaultBalance * vault.pricePerFullShare * vault.prices.eth;
      ...
      const vaultBalances = vaultsInUse.reduce(
        (accumulator, vault) => {
      ...
      const assetsInUse = assets
        ...
        .map((asset) => {
          ...
          asset.prices = price;
          asset.usdBalance =
            asset.investedBalance * asset.price * asset.prices.usd;
          asset.earnGrowth_daily_usd =
            asset.investedBalance *
            asset.price *
            (asset.maxApr / 36500) *
            asset.prices.usd;
          asset.earnGrowth_weekly_usd =
            asset.investedBalance *
            asset.price *
            (asset.maxApr / 5200) *
            asset.prices.usd;
          asset.earnGrowth_yearly_usd =
            ((asset.investedBalance * asset.price * asset.maxApr) / 100) *
            asset.prices.usd;

          asset.ethBalance =
            asset.investedBalance * asset.price * asset.prices.eth;
          asset.earnGrowth_daily_eth =
            asset.investedBalance *
            asset.price *
            (asset.maxApr / 36500) *
            asset.prices.eth;
          asset.earnGrowth_weekly_eth =
            asset.investedBalance *
            asset.price *
            (asset.maxApr / 5200) *
            asset.prices.eth;
          asset.earnGrowth_yearly_eth =
            ((asset.investedBalance * asset.price * asset.maxApr) / 100) *
            asset.prices.eth;
          return asset;
        });
      ...
      const earnBalances = assetsInUse.reduce(
        (accumulator, asset) => {
          accumulator.earnBalance_usd =
            accumulator.earnBalance_usd + asset.usdBalance;
          accumulator.earnGrowth_daily_usd =
            accumulator.earnGrowth_daily_usd + asset.earnGrowth_daily_usd;
          accumulator.earnGrowth_weekly_usd =
            accumulator.earnGrowth_weekly_usd + asset.earnGrowth_weekly_usd;
          accumulator.earnGrowth_yearly_usd =
            accumulator.earnGrowth_yearly_usd + asset.earnGrowth_yearly_usd;

          accumulator.earnBalance_eth =
            accumulator.earnBalance_eth + asset.ethBalance;
          accumulator.earnGrowth_daily_eth =
            accumulator.earnGrowth_daily_eth + asset.earnGrowth_daily_eth;
          accumulator.earnGrowth_weekly_eth =
            accumulator.earnGrowth_weekly_eth + asset.earnGrowth_weekly_eth;
          accumulator.earnGrowth_yearly_eth =
            accumulator.earnGrowth_yearly_eth + asset.earnGrowth_yearly_eth;
          return accumulator;
        },
        {
          earnBalance_usd: 0,
      ...
      const earnGrowthDailyPerc_usd =
        (earnBalances.earnGrowth_daily_usd * 100) /
        earnBalances.earnBalance_usd;
      const earnGrowthWeeklyPerc_usd =
        (earnBalances.earnGrowth_weekly_usd * 100) /
        earnBalances.earnBalance_usd;
      const earnGrowthYearlyPerc_usd =
        (earnBalances.earnGrowth_yearly_usd * 100) /
        earnBalances.earnBalance_usd;

      const earnGrowthDailyPerc_eth =
        (earnBalances.earnGrowth_daily_eth * 100) /
        earnBalances.earnBalance_eth;
      const earnGrowthWeeklyPerc_eth =
        (earnBalances.earnGrowth_weekly_eth * 100) /
        earnBalances.earnBalance_eth;
      const earnGrowthYearlyPerc_eth =
        (earnBalances.earnGrowth_yearly_eth * 100) /
        earnBalances.earnBalance_eth;

      // CALCULATE PORTFOLIO (earn + vault) BALANCES
      const portfolioBalance_usd =
        vaultBalances.vaultBalance_usd + earnBalances.earnBalance_usd;
      const portfolioGrowthDaily_usd =
        vaultBalances.vaultGrowth_daily_usd + earnBalances.earnGrowth_daily_usd;
      const portfolioGrowthWeekly_usd =
        vaultBalances.vaultGrowth_weekly_usd +
        earnBalances.earnGrowth_weekly_usd;
      const portfolioGrowthYearly_usd =
        vaultBalances.vaultGrowth_yearly_usd +
        earnBalances.earnGrowth_yearly_usd;
      const portfolioGrowthDailyPerc_usd =
        ((vaultBalances.vaultGrowth_daily_usd +
          earnBalances.earnGrowth_daily_usd) *
          100) /
        (vaultBalances.vaultBalance_usd + earnBalances.earnBalance_usd);
      ...
      const portfolioBalance_eth =
        vaultBalances.vaultBalance_eth + earnBalances.earnBalance_eth;
      const portfolioGrowthDaily_eth =
        vaultBalances.vaultGrowth_daily_eth + earnBalances.earnGrowth_daily_eth;
      const portfolioGrowthWeekly_eth =
        vaultBalances.vaultGrowth_weekly_eth +
        earnBalances.earnGrowth_weekly_eth;
      const portfolioGrowthYearly_eth =
        vaultBalances.vaultGrowth_yearly_eth +
        earnBalances.earnGrowth_yearly_eth;
      const portfolioGrowthDailyPerc_eth =
        ((vaultBalances.vaultGrowth_daily_eth +
          earnBalances.earnGrowth_daily_eth) *
          100) /
        (vaultBalances.vaultBalance_eth + earnBalances.earnBalance_eth);
