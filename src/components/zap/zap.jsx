...
class Zap extends Component {
  ...
  balancesReturned = (balances) => {
    ...
    this.setSendAsset(
      store.getStore("assets").filter((asset) => asset.curve === true)[0]
    );
  };
  ...
  onZap = () => {
    ...
    if (
      ...
      isNaN(sendAmount) ||
      sendAmount <= 0 ||
      parseFloat(sendAmount) > sendAsset.balance
    ) {
  ...
  setSendAsset = (sendAsset) => {
    ...
    this.setState({
      sendAsset,
      receiveAsset,
      sendAmount: sendAmount.toFixed(4),
    });
  };
